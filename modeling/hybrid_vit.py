...
class HybridEmbed(nn.Module):
    ...
    """
    def __init__(self, backbone, cnn_layer_index=-1, patch_size=16, img_size=224, feature_size=None, in_chans=3, embed_dim=768):
        super().__init__()
        ...
        if feature_size is None:
            with torch.no_grad():
                ...
                if training:
                    backbone.eval()
                o = self.backbone.base_forward(torch.zeros(1, in_chans, img_size[0], img_size[1]))[cnn_layer_index]
        ...
        self.num_patches = (feature_size[1] // patch_size[1]) * (feature_size[0] // patch_size[0])
...
class Hybrid_VIT(nn.Module):
    ...
    def __init__(self, num_classes=4, cnn_layer_index=-1, hybrid_backbone=None, patch_size=16, in_chans=3, depth=12,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, attn_drop_rate=0., drop_path_rate=0., embed_dim = 768,
                 norm_layer=partial(nn.LayerNorm, eps=1e-6), random_init=False, index=(2,5,8,11), mla_channels=256):
        super(Hybrid_VIT, self).__init__()
        ...
        # self.hybrid_backbone = BaseNet(nclass=num_classes, backbone='resnet50', norm_layer=nn.BatchNorm2d)
        self.hybrid_backbone = hybrid_backbone
        self.cnn_layer_index = cnn_layer_index

        self.num_stages = self.depth
        self.out_indices = tuple(range(self.num_stages))

        self.patch_embed = HybridEmbed(self.hybrid_backbone, cnn_layer_index=self.cnn_layer_index, patch_size=self.patch_size,
                                       img_size=self.img_size, in_chans=self.in_chans, embed_dim=self.embed_dim)
    ...
    def forward(self, x):
        B = x.shape[0]  # B=1
        x = self.patch_embed(x)  # [1, 768, 20, 20]
