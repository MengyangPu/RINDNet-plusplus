...
class MyNet(nn.Module):
    ...
    def __init__(self, nclass=5, backbone_name='resnet50', norm_layer=nn.BatchNorm2d):
        self.inplanes = 64
        ...
        self.backbone = BaseNet(nclass=self.nclass, backbone = self.backbone_name, norm_layer=self.norm_layer)
    ...
    def load_resnet(self):
        ...
        ignore_keys=[]
        model_dict = self.backbone.pretrained.state_dict()

        for k, v in list(pretrained_dict.items()):
            if k in ignore_keys:
                pretrained_dict.pop(k)
        model_dict.update(pretrained_dict)
        self.backbone.pretrained.load_state_dict(model_dict)

    def forward(self, x):  ## when x: (1, 3, 320, 320)
        ## resnet-50
        c1, c2, c3, c4, c5 = self.backbone.base_forward(x) #[1, 64, 320, 320]   [1, 256, 160, 160]   [1, 512, 80, 80]   [1, 1024, 40, 40]    [1, 2048, 40, 40]
...
if __name__ == '__main__':
    model = MyNet()
    dummy_input = torch.rand(1, 3, 212, 280)
