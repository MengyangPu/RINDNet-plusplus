...
class Trainer(object):
    ...
    def test(self, iters):
        ...
        for i, image in enumerate(tbar):
            ...
            illumination_pred = out_illumination.data.cpu().numpy()
            illumination_pred = illumination_pred.squeeze()
            sio.savemat(os.path.join(self.illumination_output_dir2, '{}.mat'.format(name)), {'result': illumination_pred})
    ...
    def multiscale_test(self, iters):
        ...
        for i, image in enumerate(tbar):
            ...
            sio.savemat(os.path.join(self.reflectance_output_dir, '{}.mat'.format(name)), {'result': multi_side_reflectance2})
            sio.savemat(os.path.join(self.illumination_output_dir, '{}.mat'.format(name)), {'result': multi_side_illumination2})
            '''
            ...
            sio.savemat(os.path.join(self.reflectance_output_dir2, '{}.mat'.format(name)), {'result': multi_fuse_reflectance2})
            sio.savemat(os.path.join(self.illumination_output_dir2, '{}.mat'.format(name)), {'result': multi_fuse_illumination2})
